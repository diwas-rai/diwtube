...
export const videosRouter = createTRPCRouter({
        ...
        const [existingVideo] = await db
            ...
            .select({
                ...
                user: {
                    ...
                    viewerSubscribed: isNotNull(viewerSubscriptions.viewerId).mapWith(Boolean), // workaround to make this boolean instead of unknown
                },
                viewCount: db.$count(videoViews, eq(videoViews.videoId, videos.id)),
                likeCount: db.$count(
                    videoReactions,
                    and(eq(videoReactions.videoId, videos.id), eq(videoReactions.reaction, "like"))
                ),
                dislikeCount: db.$count(
                    videoReactions,
                    and(eq(videoReactions.videoId, videos.id), eq(videoReactions.reaction, "dislike"))
                ),
    ...
    generateThumbnail: protectedProcedure
        .input(z.object({ id: z.string().uuid(), prompt: z.string().min(10).max(100) }))
