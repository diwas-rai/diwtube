...
export const videosRouter = createTRPCRouter({
        ...
        .select({
          ...
          user: {
            ...
            viewerSubscribed: isNotNull(viewerSubscriptions.viewerId).mapWith(
              Boolean
            ), // workaround to make this boolean instead of unknown
          },
  ...
  generateThumbnail: protectedProcedure
    .input(
      z.object({ id: z.string().uuid(), prompt: z.string().min(10).max(100) })
    )
  ...
  revalidate: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const textTrack = asset.tracks?.filter((track) => track.type === "text");
      ...
      const [updatedVideo] = await db
        ...
        .set({
          ...
          muxTrackId: textTrack?.[0].id ?? null,
          muxTrackStatus: textTrack?.[0].status ?? "errored",
        })
        .where(and(eq(videos.id, input.id), eq(videos.userId, userId)))
        .returning();
