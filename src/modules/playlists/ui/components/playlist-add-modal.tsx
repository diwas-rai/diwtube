...
export const PlaylistAddModal = ({
  ...
  const {
    ...
    isFetchingNextPage,
    fetchNextPage,
  } = trpc.playlists.getManyWithVideoStatus.useInfiniteQuery(
    {
      limit: DEFAULT_LIMIT,
      videoId,
    },
  ...
  const addVideo = trpc.playlists.addVideo.useMutation({
    onSuccess: (data) => {
      ...
      utils.playlists.getManyWithVideoStatus.invalidate({ videoId });
  ...
  const handleOpenChange = (open: boolean) => {
    utils.playlists.getManyWithVideoStatus.refetch({
      videoId,
      limit: DEFAULT_LIMIT,
    });
    onOpenChange(open);
  ...
  const handleOnClick = (
    ...
    if (isVideoInPlaylist) {
      ...
      addVideo.mutate({ videoId, playlistId });
    }
  };
          ...
          playlists?.pages
            ...
            .map((playlist) => (
              <Button
                ...
                size="lg"
                onClick={() =>
                  handleOnClick(playlist.containsVideo, videoId, playlist.id)
                }
